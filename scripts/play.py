...
RL_SCRIPT = ISAACLAB_ROOT / "scripts" / "reinforcement_learning" / "rsl_rl" / "play_rsl_rl.py"


def main() -> None:
    if not RL_SCRIPT.exists():
        raise FileNotFoundError(f"Could not find Isaac Lab play script at: {RL_SCRIPT}")
    ...
    from h1_carry_box_task import PLAY_TASK_ID  # noqa: PLC0415

    if "--task" not in argv:
        argv = ["--task", PLAY_TASK_ID, *argv]

    sys.argv = [str(RL_SCRIPT), *argv]
    runpy.run_path(str(RL_SCRIPT), run_name="__main__")
